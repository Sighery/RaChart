...
def store_search(title):
	payload = {
		"f": "games",
		"term": title
	}

	req = requests.get("http://store.steampowered.com/search/suggest", params = payload)

	soup = BeautifulSoup(req.text, 'html.parser')

	for elem in soup.find_all(class_ = "ds_collapse_flag"):
		if elem.find(class_ = "match_name").get_text() == title:
			return u"**[%s](http://store.steampowered.com/app/%s/)**" % (title, elem['data-ds-appid'])
...
try:
	...
	new_file.write("GAME | \n")
	new_file.write(":- | \n")
	with open("list.txt") as titles_file:
		for line in titles_file:
			title = store_search(line.rstrip("\n"))
			new_file.write(title.encode("UTF-8") + " | \n")

	new_file.close()
	print "A new file called 'chart.txt' has been created on this same directory"
	bcompleted = True
except:
	print "Couldn't find any list.txt file for the automatic generation of the chart, proceeding with fallback on manual mode."

if bcompleted is False:
	try:
